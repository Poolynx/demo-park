...
import com.wesleyRabelo.demoparkapi.service.ClienteVagaService;
import com.wesleyRabelo.demoparkapi.service.EstacionamentoService;
import com.wesleyRabelo.demoparkapi.service.JasperService;
...
import com.wesleyRabelo.demoparkapi.web.dto.mapper.ClienteVagaMapper;
...
public class EstacionamentoController {
    ...
    @Operation(summary = "Operação de check-in", description = "Recurso para dar entrada de um veículo no estacionamento" +
            "Requisição exige uso de um bearer token. Uso exclusivo da ROLE= 'ADMIN'",
    ...
    @Operation(summary = "Operação de check-out", description = "Recurso para dar saída de um veículo no estacionamento" +
            ...
            responses = {
                    ...
                    @ApiResponse(responseCode = "404", description = "Possíveis causas: <br/>" +
                            ...
                            "- Veículo já realizou o check-out;",
                            content = @Content(mediaType = "application/json;charset=UTF-8",
                                    schema = @Schema(implementation = ErrorMessage.class))),
            })
    ...
    @GetMapping("/cpf/{cpf}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<PageableDto> getAllEstacionamentoPorCpf(@PathVariable String cpf, @Parameter(hidden = true)
    ...
    public ResponseEntity<PageableDto> getAllEstacionamentoDoCliente(@AuthenticationPrincipal JwtUserDetails user,
        ...
        Page<ClienteVagaProjection> projection = clienteVagaService.buscarTodosPorUusarioId(user.getId(), pageable);
        PageableDto dto = PageableMapper.toDto(projection);
        return ResponseEntity.ok(dto);
    }
